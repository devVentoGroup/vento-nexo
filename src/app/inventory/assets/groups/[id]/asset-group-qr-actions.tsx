...
function safeFileName(value: string) {
  return value
    ...
    .replace(/^-+|-+$/g, "")
    .toLowerCase()
    .slice(0, 80) || "grupo-activos";
}

export function AssetGroupQrActions({
  qrUrl,
  assetUrl,
  groupTitle,
  groupCode,
}: AssetGroupQrActionsProps) {
  const printQr = () => {
    ...
    printWindow.document.write(`
      ...
      <html>
        <head>
          <title>QR ${groupCode || groupTitle}</title>
          <style>
            body {
  ...
  const downloadQr = async () => {
    try {
      ...
      anchor.download = `qr-${safeFileName(groupCode || groupTitle)}.png`;
