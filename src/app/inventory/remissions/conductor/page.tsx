...
export default async function ConductorShipmentsPage({ searchParams }: { searchParams?: Promise<{ ok?: string; error?: string }> }) {
  ...
  const renderShipment = (shipment: Shipment) => (
    <article key={shipment.id} className="rounded-2xl border border-[var(--ui-border)] bg-white p-4 shadow-sm">
      ...
      {shipment.status !== "in_transit" ? <form action={departPhysicalShipment} className="mt-4"><input type="hidden" name="shipment_id" value={shipment.id} /><button className="ui-btn ui-btn--brand">Confirmar salida</button></form> : <p className="mt-4 ui-caption">La recepción se confirmará desde la sede destino.</p>}
    </article>
  );
