...
export function FulfillmentRouteSelectors({ sites, products, locations, areasBySite, productSiteSettings, productionRoutes, defaults }: Props) {
  ...
  const destinationAreas = (destinationSetting?.areaKinds.length ? destinationSetting.areaKinds.map((code) => (areasBySite[toSiteId] ?? []).find((area) => area.code === code) ?? { code, name: code }) : areasBySite[toSiteId] ?? []);
  ...
  return <>
    ...
    <label className="flex flex-col gap-1"><span className="ui-label">Área que solicita</span><select name="requesting_area_kind" className="ui-input" value={requestingAreaKind} disabled={!toSiteId} onChange={(event) => setRequestingAreaKind(event.target.value)}><option value="">{toSiteId ? "Cualquier área configurada" : "Primero selecciona sede destino"}</option>{destinationAreas.map((area) => <option key={area.code} value={area.code}>{area.name ?? area.code}</option>)}</select></label>
